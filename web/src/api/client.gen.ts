...
export const WebRPCSchemaHash = "739162b7e2088c7596aea8168dc19abc7a6441f5"
...
export interface FileService {
  fileList(args: FileListArgs, headers?: object, signal?: AbortSignal): Promise<FileListReturn>
}
...
export class FileService implements FileService {
  ...
  fileList = (args: FileListArgs, headers?: object, signal?: AbortSignal): Promise<FileListReturn> => {
    return this.fetch(
      ...
      return buildResponse(res).then(_data => {
        return {
          res: <FileListResponse>(_data.res),
        }
      })
    }, (error) => {
      throw WebrpcRequestFailedError.new({ cause: `fetch(): ${error.message || ''}` })
