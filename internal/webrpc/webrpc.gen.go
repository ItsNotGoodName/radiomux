// radiomux v0.1.0 739162b7e2088c7596aea8168dc19abc7a6441f5
// --
...
func WebRPCSchemaHash() string {
	return "739162b7e2088c7596aea8168dc19abc7a6441f5"
}
...
type PlayerService interface {
	...
	PlayerWsURL(ctx context.Context, id int64) (string, error)
...
func (s *playerServiceServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "/rpc/PlayerService/PlayerWsURL":
		handler = s.servePlayerWsURLJSON
...
func (s *playerServiceServer) servePlayerWsURLJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	ctx = context.WithValue(ctx, MethodNameCtxKey, "PlayerWsURL")
	...
	// Call service method implementation.
	ret0, err := s.PlayerService.PlayerWsURL(ctx, reqPayload.Arg0)
	if err != nil {
		rpcErr, ok := err.(WebRPCError)
		if !ok {
			rpcErr = ErrWebrpcEndpoint.WithCause(err)
...
func NewFileServiceServer(svc FileService) *fileServiceServer {
	return &fileServiceServer{
		FileService: svc,
	}
}

func (s *fileServiceServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	defer func() {
		// In case of a panic, serve a HTTP 500 error and then panic.
		if rr := recover(); rr != nil {
			s.sendErrorJSON(w, r, ErrWebrpcServerPanic.WithCause(fmt.Errorf("%v", rr)))
	...
	var handler func(ctx context.Context, w http.ResponseWriter, r *http.Request)
	switch r.URL.Path {
	case "/rpc/FileService/FileList":
		handler = s.serveFileListJSON
	default:
		err := ErrWebrpcBadRoute.WithCause(fmt.Errorf("no handler for path %q", r.URL.Path))
		s.sendErrorJSON(w, r, err)
		return
...
func (s *fileServiceServer) serveFileListJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	ctx = context.WithValue(ctx, MethodNameCtxKey, "FileList")
	...
	// Call service method implementation.
	ret0, err := s.FileService.FileList(ctx, reqPayload.Arg0)
	if err != nil {
		rpcErr, ok := err.(WebRPCError)
		if !ok {
			rpcErr = ErrWebrpcEndpoint.WithCause(err)
	...
	respPayload := struct {
		Ret0 *FileListResponse `json:"res"`
	}{ret0}
	respBody, err := json.Marshal(respPayload)
	if err != nil {
		s.sendErrorJSON(w, r, ErrWebrpcBadResponse.WithCause(fmt.Errorf("failed to marshal json response: %w", err)))
...
func (s *fileServiceServer) sendErrorJSON(w http.ResponseWriter, r *http.Request, rpcErr WebRPCError) {
	if s.OnError != nil {
		s.OnError(r, &rpcErr)
	}
...
var (
	...
	ErrNotImplemented = WebRPCError{Code: 101, Name: "NotImplemented", Message: "not implemented", HTTPStatus: 501}
)
